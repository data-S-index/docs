...
> **These S-index scores were computed automatically for demo and validation purposes only.** They are not intended to represent a researcher's definitive S-index. For an accurate S-index, researchers should [create a profile](https://beta.scholardata.io/signup), claim their datasets, and let Scholar Data compute their score from a verified, curated dataset list. The process to compute the S-index for those researcher created profiles remains the same.
...
Each author's S-index was calculated as the sum of the D-index scores of all datasets attributed to them, following the [S-index formula](../concepts). The auto-generated profiles and their S-index scores are browsable on Scholar Data's [author search page](https://beta.scholardata.io/search/au).

...
D-index and S-index scores so they remain comparable across disciplines. The idea is that a dataset in
a field with low data sharing rates and inconsistent citation practices should not be penalized relative to one in a field
where sharing and citation is common practice.

## Classification System

Research fields follow the
[OpenAlex topics and domains taxonomy](https://docs.openalex.org/api-entities/topics),
a four-level hierarchy of 4,516 topics grouped into 252 subfields, 26 fields, and 4
top-level domains. Scholar Data uses the **subfield level** for normalization, as it
...
- **Custom classifier**: for datasets not indexed in OpenAlex, a custom topic
  ...
  [GitHub repository](https://github.com/data-S-index/dataset-to-field).

A small number of datasets with non-Latin script metadata could not be classified and
remain unassigned.
...
interdisciplinary datasets. In the future, we will implement a process to for users to suggest a correction.
